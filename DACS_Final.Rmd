...
```{r step13, echo=FALSE, fig.align='center', out.width='50%', fig.cap="Calinski–Harabasz"}
...
```{r step14, echo=FALSE, fig.align='center', out.width='50%', fig.cap="Davies–Bouldin"}
...
**Cách hoạt động của PCA:**
  ...
  - **3. Tính vectơ riêng và giá trị riêng:** Vectơ riêng cho biết hướng chính, giá trị riêng thể hiện mức độ biến động theo hướng đó.
  
  - **4. Lựa chọn thành phần chính:** Các vectơ riêng ứng với giá trị riêng lớn nhất được chọn làm thành phần chính, giúp giảm số chiều dữ liệu trong khi giữ lại phần lớn thông tin quan trọng.
...
Thuật toán UMAP được xây dựng trên nền tảng lý thuyết tô pô đại số và hình học vi phân, với hai giả định chính:
  ...
  - Cấu trúc địa phương của dữ liệu có thể được bảo toàn thông qua xây dựng biểu đồ kề.
...
UMAP được sử dụng phổ biến trong các lĩnh vực như sinh học phân tử, tài chính, và phân tích hành vi người dùng – nơi dữ liệu có cấu trúc phức tạp và nhiều chiều. Nhờ khả năng bảo toàn tốt cả cấu trúc cục bộ lẫn toàn cục, UMAP thường được dùng để trực quan hóa và hỗ trợ phân cụm sau bước tiền xử lý dữ liệu. Ngoài ra, UMAP còn nổi bật với tốc độ nhanh, khả năng mở rộng cao, phù hợp cho các hệ thống khai phá dữ liệu quy mô lớn.[@noauthor_umap_nodate]
...
Biểu đồ UMAP giúp trực quan hóa kết quả phân cụm trong không gian 2 chiều. Tuy nhiên, **các cụm khách hàng chưa được tách biệt rõ**, nhiều điểm bị **chồng lấn** hoặc **giao nhau**, đặc biệt là giữa cụm 1 và cụm 2.

Điều này cho thấy **các nhóm khách hàng vẫn còn tương đồng về đặc điểm**, hoặc UMAP chưa giữ được hoàn toàn cấu trúc phân cụm từ không gian ban đầu.

Tuy vậy, UMAP vẫn mang lại giá trị trong việc giữ nguyên cấu trúc lân cận giữa các điểm, từ đó phản ánh độ liên kết và mật độ phân bố khách hàng trong toàn bộ tập dữ liệu. Kết quả này có thể giúp doanh nghiệp cân nhắc các nhóm khách hàng cận biên và xây dựng các chính sách tiếp cận linh hoạt hơn.
